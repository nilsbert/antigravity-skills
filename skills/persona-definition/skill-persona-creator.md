...
3.  **Avatar:** Ask the user if they want to generate a portrait.
    *   **If Yes:**
        *   Generate an image prompt (Style: *"3D Pixar comic style portrait..."*).
        *   Save the image to: `docs/product/personas/[persona_name_slug]/avatar.png`.
        *   Replace `{AVATAR_PATH}` in the draft with `./avatar.png`.
    *   **If No:**
        *   Replace `{AVATAR_PATH}` with a placeholder or remove the line.
